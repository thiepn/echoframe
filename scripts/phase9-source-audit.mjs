...
const REPORT = path.join(ROOT, 'docs', 'PHASE9_SOURCE_AUDIT.json');
...
for (const file of sourceFiles) {
  ...
  const generationScope = rel.startsWith('src/encounter/')
    ...
    || rel.startsWith('src/scoring/')
    || rel.startsWith('src/progression/')
    || rel.startsWith('src/statistics/')
...
console.log(`Phase 9 source audit: ${report.passed ? 'PASS' : 'FAIL'}`);
