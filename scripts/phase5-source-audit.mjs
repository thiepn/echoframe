...
const REPORT = path.join(ROOT, 'docs', 'PHASE5_SOURCE_AUDIT.json');
...
for (const file of sourceFiles) {
  ...
  if (rel !== 'src/components/HealthComponent.js') {
    const healthPattern = /(?:\.health\.(?:currentHealth|maximumHealth)|currentHealth|maximumHealth)\s*(?:=|\+=|-=|\*=|\/=)/g;
    let healthMatch;
...
console.log(`Phase 5 source audit: ${report.passed ? 'PASS' : 'FAIL'}`);
