...
export async function productionBundleDigest(directory = path.join(ROOT, 'dist'), { exclude = [] } = {}) {
  if (!(await fileExists(directory))) return null;
  const excluded = new Set(exclude);
  const files = (await walk(directory, directory)).filter((relative) => !excluded.has(relative)).sort();
  const entries = [];
  ...
  return {
    ...
    digest: sha256Buffer(entries.map((entry) => `${entry.sha256} ${entry.bytes} ${entry.path}`).join('\n')),
